...
import type { AcademicReportPayload, AcademicReportsPack } from './academicServices';
...
function appendReportSheets(wb: XLSX.WorkBook, report: AcademicReportPayload, usedNames: Set<string>) {
...
export function downloadAcademicReportExcel(report: AcademicReportPayload) {
  ...
  XLSX.writeFile(wb, `Academic_${safeType}_${year}.xlsx`);
}

export function downloadAllAcademicReportsExcel(pack: AcademicReportsPack) {
  const wb = XLSX.utils.book_new();
  const used = new Set<string>();
  ...
  const term = pack.term.replace(/[^a-zA-Z0-9_-]+/g, '_');
  XLSX.writeFile(wb, `Academic_All_Reports_${year}_${term}.xlsx`);
}
