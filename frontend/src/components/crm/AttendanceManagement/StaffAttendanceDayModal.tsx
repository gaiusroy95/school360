...
import { fetchStaffAttendanceDay, type StaffDayDetail } from '../../../lib/attendanceServices';
import { downloadStaffDayDetailExcel } from '../../../lib/staffAttendanceExcel';
...
export function StaffAttendanceDayModal({ open, onClose, date, academicYear }: Props) {
  const [detail, setDetail] = useState<StaffDayDetail | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setDetail(await fetchStaffAttendanceDay({ date, academicYear }));
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/45 backdrop-blur-[3px]" onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100 shrink-0 gap-3">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Staff Attendance — Day Detail</h3>
            <p className="text-sm text-slate-600 mt-0.5">{detail?.dateLabel || date}</p>
            <p className="text-[10px] text-slate-400 mt-1 flex items-center gap-1">
              <Smartphone size={10} />
              Data captured from Staff mobile app
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {detail && (
              <button
                type="button"
                onClick={() => downloadStaffDayDetailExcel(detail)}
              ...
              {detail.sections.map((section) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs font-semibold text-slate-500">{section.count} Staff{section.count === 1 ? '' : 's'}</span>
                  </div>
                  {section.count === 0 ? (
                    <p className="text-xs text-slate-400 italic">No staff in this category</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="text-slate-500 border-b border-slate-200/60">
                            <th className="text-left py-2 pr-3 font-semibold">Staff</th>
                          ...
                          {(section.staff || []).map((t) => (
                            <tr key={t.id} className="border-b border-slate-100/80">
                              <td className="py-2 pr-3">
                                <p className="font-semibold text-slate-800">{t.staff.staffName}</p>
                                <p className="text-[10px] text-slate-400">{t.staff.employeeCode}</p>
                              </td>
                              <td className="py-2 pr-3 text-slate-600">{t.staff.department}</td>
                              <td className="py-2 pr-3 text-slate-600">{t.checkInTime || '—'}</td>
                              <td className="py-2 text-slate-600">{t.staffRemarks || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {detail.unmarkedCount > 0 && (
                <div className="rounded-xl border border-dashed border-slate-300 p-4 bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-500">
                    {detail.unmarkedStaff.map((t) => t.staffName).join(', ')}
                  </p>
                </div>
              )}
            </>
