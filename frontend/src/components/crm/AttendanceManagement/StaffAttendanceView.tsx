...
import { StaffAttendanceDayModal } from './StaffAttendanceDayModal';
...
export function StaffAttendanceView() {
  const now = new Date();
  const [meta, setMeta] = useState<Awaited<ReturnType<typeof fetchStaffAttendanceMeta>> | null>(null);
  const [calendar, setCalendar] = useState<StaffCalendar | null>(null);
  const [report, setReport] = useState<StaffAttendanceReport | null>(null);
  const [academicYear, setAcademicYear] = useState('2025-26');
  const [period, setPeriod] = useState<StaffPeriod>('monthly');
  ...
  const load = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      const m = await fetchStaffAttendanceMeta();
      setMeta(m);
      setAcademicYear(m.defaultAcademicYear);
      const [cal, rep] = await Promise.all([
        fetchStaffAttendanceCalendar({ academicYear: m.defaultAcademicYear, year: calYear, month: calMonth }),
        fetchStaffAttendanceReport({
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Failed to load staff attendance');
    } finally {
      setLoading(false);
    }
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const [cal, rep] = await Promise.all([
        fetchStaffAttendanceCalendar({ academicYear, year: calYear, month: calMonth }),
        fetchStaffAttendanceReport(reportParams),
      ]);
      setCalendar(cal);
      setReport(rep);
    } catch (err) {
  ...
  const calendarCells = useMemo(() => {
    ...
    const cells: (StaffCalendar['days'][0] | null)[] = Array(startPad).fill(null);
    ...
    <div className="flex flex-col h-full min-h-0 bg-gradient-to-b from-slate-50 to-slate-100/90">
      <div className="px-4 md:px-6 py-4 bg-white border-b border-slate-200/90 shrink-0">
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Attendance Management › Staff Attendance</p>
            <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-0.5">Staff Attendance</h1>
            <p className="text-xs text-slate-500 mt-1 flex items-center gap-1.5">
              <Smartphone size={12} className="text-indigo-500" />
              {meta?.captureNote || 'Attendance and leave captured from Staff mobile app only'}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <div>
              ...
              <select value={period} onChange={(e) => setPeriod(e.target.value as StaffPeriod)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase">Total Staff</p>
              <p className="text-2xl font-bold text-slate-900">{calendar?.totalStaff ?? meta?.staffCount ?? 0}</p>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-1">
              {(report?.categoryExports || []).map((cat) => (
                <button
                  key={cat.key}
                  type="button"
                  onClick={() => report && downloadStaffAttendanceReportExcel(report, cat.key)}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-[10px] text-slate-500">Click any date to view staff in all five categories</p>
            </div>
              ...
              <table className="w-full text-[10px] min-w-[900px]">
                <thead className="sticky top-0 bg-slate-50 z-10">
                  <tr className="text-slate-500 border-b border-slate-200">
                    <th className="px-3 py-2 text-left font-semibold sticky left-0 bg-slate-50 z-20 min-w-[140px]">Staff</th>
                ...
                <tbody>
                  {(report?.rows || []).map((row) => (
                    <tr key={row.staff.id} className="border-b border-slate-50 hover:bg-slate-50/80">
                      <td className="px-3 py-2 sticky left-0 bg-white z-10">
                        <p className="font-semibold text-slate-800">{row.staff.staffName}</p>
                        <p className="text-[9px] text-slate-400">{row.staff.employeeCode}</p>
                      </td>
                      <td className="px-2 py-2 text-slate-600">{row.staff.department}</td>
                      {(report?.dateColumns || []).map((d) => (
